...
INSERT INTO `comments` (`comment_text`, `customer_id`, `product_id`, `order_id`, `created_at`, `updated_at`) VALUES
('Great product, very satisfied!', 1, 1, 1, '2024-06-15 10:00:00', '2024-06-15 10:00:00'),
('The quality is amazing, will buy again.', 2, 3, 2, '2024-06-16 11:00:00', '2024-06-16 11:00:00'),
('Not what I expected, but okay.', 3, 4, 3, '2024-06-17 12:00:00', '2024-06-17 12:00:00'),
('Arrived late, but product is good.', 4, 5, 4, '2024-06-18 13:00:00', '2024-06-18 13:00:00'),
('Excellent customer service!', 5, 6, 5, '2024-06-19 14:00:00', '2024-06-19 14:00:00'),
('Love this product, highly recommend.', 6, 7, 6, '2024-06-20 15:00:00', '2024-06-20 15:00:00'),
('Good value for money.', 7, 8, 7, '2024-06-21 16:00:00', '2024-06-21 16:00:00'),
('Not satisfied, will return.', 8, 9, 8, '2024-06-22 17:00:00', '2024-06-22 17:00:00'),
('Perfect for my needs.', 9, 10, 9, '2024-06-23 18:00:00', '2024-06-23 18:00:00'),
('Good product but packaging was damaged.', 10, 11, 10, '2024-06-24 19:00:00', '2024-06-24 19:00:00');
...
,`total_amount_ordered` decimal(10,2)
,`order_status` enum('PENDING','CONFIRMED','PROCESSING','SHIPPED','DELIVERED','CANCELLED','RETURNED','REFUNDED','FAILED','ON_HOLD')
,`quantity` int(11)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `product_details`  AS SELECT `p`.`id` AS `product_id`, `p`.`name` AS `product_name`, `p`.`price` AS `product_price`, `c`.`category_name` AS `category_name`, `s`.`company_name` AS `company_name`, `o`.`id` AS `order_id`, `o`.`customer_id` AS `customer_id`, `o`.`total_amount` AS `total_amount_ordered`, `o`.`order_status` AS `order_status`, `os`.`quantity` AS `quantity`, `o`.`created_at` AS `order_date` FROM (((((`products` `p` left join `categories` `c` on(`p`.`category_id` = `c`.`id`)) left join `product_suppliers` `ps` on(`p`.`id` = `ps`.`product_id`)) left join `suppliers` `s` on(`ps`.`supplier_id` = `s`.`id`)) left join `order_segments` `os` on(`p`.`id` = `os`.`product_id`)) left join `orders` `o` on(`os`.`order_id` = `o`.`id`)) ;
...
ALTER TABLE `customers`
  ...
  ADD KEY `idx_last_anme` (`last_name`);
...
ALTER TABLE `returned_orders`
  ...
  ADD KEY `product_id` (`product_id`),
  ADD KEY `order_id` (`order_id`),
  ADD KEY `idx_returned_date` (`returned_date`);
